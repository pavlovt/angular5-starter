...
export class ValidationManager {
  ...
  constructor(
    ...
    _.each(this.inputsRaw, (input, key) => {
    // for (var key in this.inputsRaw) {
      /*if (_.isArray(input)) {
        let controls = [];
        _.each(input, (inp, key) => {
          controls[key] = this.buildControl(key, inp);
        });
        this.controls[key] = _.cloneDeep(controls);
        this.formControls[key] = new FormArray(_.map(controls, ctrl => ctrl.control));
        this.errors[key] = '';
      } else {*/
        this.controls[key] = this.buildControl(key, input);
...
export const VALIDATION_MESSAGES = {
  ...
  'passwordcheck'     : 'Password should have at least 6 characters , max 18 characters A-Z, a-z, 0-9, special symbols except  “<”, “>” ',
  'nospace'           : 'This field cannot contain only spaces'
};
